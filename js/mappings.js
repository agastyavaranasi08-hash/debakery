...
function setupMappingTable() {
  ...
  const tableBody = document.getElementById('mapping-rows');
...
function renderArcDetails() {
  ...
  if (!arc || !metaCard || !mappingsCard || !uploadCard) {
    hideArcSections();
    return;
  }

  metaCard.hidden = false;
  mappingsCard.hidden = false;
  ...
  renderArcHealth();
  renderMappingsTable(arc);
}

function renderMappingsTable(arc) {
  const tableBody = document.getElementById('mapping-rows');
  if (!tableBody) return;
  ...
  if (!arc.mappings.length) {
    tableBody.innerHTML = `
      <tr>
        <td colspan="6">
          <p class="hint">No mappings yet. Add a row to begin aligning content.</p>
        </td>
      </tr>
    `;
    return;
    ...
    .map(
      (mapping) => `
        <tr data-id="${mapping.id}">
          <td><input type="text" data-field="label" value="${escapeHtml(mapping.label)}" /></td>
          <td><input type="text" data-field="manga" value="${escapeHtml(mapping.manga)}" /></td>
          <td><input type="text" data-field="ln" value="${escapeHtml(mapping.ln)}" /></td>
          <td><input type="text" data-field="anime" value="${escapeHtml(mapping.anime)}" /></td>
          <td><textarea data-field="notes" rows="2">${escapeHtml(mapping.notes)}</textarea></td>
          <td>
...
function hideArcSections() {
  ...
  document.getElementById('mappings-card')?.setAttribute('hidden', 'true');
